...
//			printf("st_shift_cust: %f", st_shift_cust);
//			printf("st_shift: %f", st_shift[jj]);
        if (st_shift[jj]) {
...
int mpu_run_self_test(long *gyro, long *accel)
{
    ...
    gyro_result = gyro_self_test(gyro, gyro_st);
		for(ii=0;ii<3;ii++){
			printf("accel_st: %ld", accel_st[ii]);
			printf("accel: %ld", accel[ii]);
		}
		printf("accel_result: %d", accel_result);
		printf("gyro_result: %d", gyro_result);
 ...
 *  @return     0 if successful.
 */
int mpu_get_compass_reg(short *mag, unsigned long *timestamp)
{
#ifdef AK89xx_SECONDARY
    unsigned char tmp[9];
    if (!(st.chip_cfg.sensors & INV_XYZ_COMPASS))
