    ...
    unsigned long *timestamp, short *sensors, unsigned char *more)
{
    ...
    /* Get a packet. */
	//main函数中调用dmp_enable_feature函数就会确定packet_length
	//目前长度是32字节，more代表还可以读几次数据，即几个32
	//fifo_data前16个字节是4个硬件解算出来的四元数（long型），再接着是
	//3个加速度计测量值和3陀螺仪测量值（都是short型），这就是16+12字节了，剩下的4个字节
	//无关紧要。
